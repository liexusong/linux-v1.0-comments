...
static struct sk_buff *ip_defrag(struct iphdr *iph, struct sk_buff *skb, struct device *dev)
{
    ...
    /* Find the entry of this IP datagram in the "incomplete datagrams" queue. */
       ...
       ptr = skb->data + dev->hard_header_len + ihl; // 指向数据
...
ip_rcv(struct sk_buff *skb, struct device *dev, struct packet_type *pt)
{
  ...
  // 0x0020 二进制为: 00100000 00000000
  if (iph->frag_off & 0x0020)
      is_frag|=1; // 是否最后一个分片
  // 0x1fff 二进制为: 00011111 11111111
  if (ntohs(iph->frag_off) & 0x1fff)
      is_frag|=2; // 是否有偏移量

  /* Do any IP forwarding required.  chk_addr() is expensive -- avoid it someday. */
  if ((brd = chk_addr(iph->daddr)) == 0) { // IP地址不属于本机
#ifdef CONFIG_IP_FORWARD
    ip_forward(skb, dev, is_frag); // 转发操作
#else
  ...
  if(is_frag) // 是否为分片包(第一个分片为0)
  {
#ifdef CONFIG_IP_DEFRAG
    skb=ip_defrag(iph,skb,dev); // 合并分片
    if(skb==NULL) {
        return 0;
    }
    iph=skb->h.iph;
  ...
  skb->ip_hdr = iph;
  skb->h.raw += iph->ihl*4; // 指向上层协议头(如TCP/UDP)
